...
**Architecture:** Add per-position desktop `max-width` rules to each `.index-card__inner`, leaving the grid tracks and all image intrinsic-ratio rules unchanged. Protect the behavior with a focused source regression test, then verify rendered dimensions in a real browser at wide desktop and mobile widths.
...
Do not modify `.index-card__img`, `.index-grid__inner`, homepage hero selectors, grid columns, or information transforms.
